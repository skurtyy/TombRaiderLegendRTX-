...
def cmd_codegen(args: argparse.Namespace) -> None:
    ...
    if args.output:
        ...
        print(
            f"Generated {args.output} ({label}, {SLOT_COUNT} methods, max {max_argc()} args)"
        )
    else:
...
def cmd_trigger(args: argparse.Namespace) -> None:
    ...
    if delay > 0:
        for remaining in range(delay, 0, -1):
            print(
                f"\r  Triggering capture in {remaining}s... (alt-tab to game now) ",
                end="",
                flush=True,
            )
            time.sleep(1)
        ...
        if progress_path.exists():
            try:
                ...
                if frame != last_frame or seq != last_seq:
                    ...
                    print(
                        f"\r  [frame {frame}] {seq} calls captured ({elapsed:.1f}s)  ",
                        end="",
                        flush=True,
                    )
    ...
    if jsonl_path.exists():
        ...
        print(
            f'  Run: python -m graphics.directx.dx9.tracer analyze "{jsonl_path}" --summary'
        )
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    cg.add_argument(
        "--output", "-o", default=None, help="Output path (default: stdout)"
    )
    cg.add_argument(
        "--format",
        "-f",
    ...
    tr.add_argument(
        "--game-dir", "-d", default=None, help="Target directory (default: cwd)"
    )
    tr.add_argument(
        "--frames", "-n", type=int, default=2, help="Number of frames to capture"
    )
    tr.add_argument(
        "--delay", type=int, default=3, help="Countdown seconds before trigger"
    )
    ...
    an.add_argument(
        "--summary", action="store_true", help="Overview: calls per frame/method"
    )
    an.add_argument(
        "--draw-calls", action="store_true", help="List draws with state deltas"
    )
    an.add_argument("--callers", metavar="METHOD", help="Caller histogram for a method")
    an.add_argument(
        "--hotpaths", action="store_true", help="Frequency-sorted call paths"
    )
    an.add_argument(
        "--top", type=int, default=20, help="Number of results for ranked output"
    )
    an.add_argument(
        "--state-at", type=int, metavar="SEQ", help="Reconstruct state at sequence N"
    )
    an.add_argument(
        "--render-loop", action="store_true", help="Detect render loop entry point"
    )
    an.add_argument(
        "--render-passes", action="store_true", help="Group draws by render target"
    )
    an.add_argument(
        "--matrix-flow", action="store_true", help="Track constant matrix uploads"
    )
    an.add_argument(
        "--shader-map",
        action="store_true",
        help="Disassemble shaders, show register map",
    )
    an.add_argument(
        "--fxc", metavar="PATH", help="Path to fxc.exe (fallback for shader disasm)"
    )
    an.add_argument(
        "--diff-draws",
        nargs=2,
    ...
    an.add_argument(
        "--rt-graph", action="store_true", help="Render target dependency graph"
    )
    an.add_argument(
        "--classify-draws", action="store_true", help="Auto-tag draws by render state"
    )
    an.add_argument(
        "--vtx-formats", action="store_true", help="Group draws by vertex declaration"
    )
    an.add_argument(
        "--redundant", action="store_true", help="Find redundant state-set calls"
    )
    an.add_argument(
        "--texture-freq", action="store_true", help="Texture binding frequency"
    )
    an.add_argument(
        "--const-provenance",
        action="store_true",
    ...
    an.add_argument(
        "--animate-constants", action="store_true", help="Cross-frame constant tracking"
    )
    an.add_argument(
        "--pipeline-diagram",
        action="store_true",
        help="Auto-generate mermaid render pipeline",
    )
    an.add_argument(
        "--resolve-addrs", metavar="BINARY", help="Resolve backtrace addrs via retools"
    )
