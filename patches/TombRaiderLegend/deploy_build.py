...
def apply_drawcache_crash_fix(src_path: Path) -> bool:
    ...
    if "DrawCache_Record" not in text:
        print(
            "  [crash-fix] No DrawCache found — skipping (builds 069-071 don't have it)"
        )
        return False
    ...
    if (
        old_heap in text
        and "DrawCache_Clear"
        not in text.split(old_heap)[0].split(
            "static unsigned long __stdcall WD_Release"
        )[-1]
    ):
    ...
    if old_raw_release in text:
        ...
        changed = True
        print(
            "  [crash-fix] Replaced raw s_drawCacheCount=0 in WD_Release with DrawCache_Clear"
        )
    ...
    if old_raw_reset in text:
        ...
        changed = True
        print(
            "  [crash-fix] Replaced raw s_drawCacheCount=0 in WD_Reset with DrawCache_Clear"
        )
    ...
    if old_raw_begin in text:
        ...
        changed = True
        print(
            "  [crash-fix] Replaced raw s_drawCacheCount=0 in WD_BeginScene with DrawCache_Clear"
        )
    ...
    if changed:
        ...
        print(
            "  [crash-fix] No matching patterns found — file may already be patched or use different code"
        )
