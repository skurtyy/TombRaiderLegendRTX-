...
namespace shared::common
{
	...
	public:
		...
		// --- TRL shader passthrough ---

		// Apply W/V/P transforms via SetTransform while keeping shaders active.
		// Reads View/Proj from game memory, computes World = WVP * inv(VP).
		void apply_transforms_passthrough(IDirect3DDevice9* dev);

		bool world_dirty() const { return world_dirty_; }
		bool view_proj_dirty() const { return view_proj_dirty_; }
	...
	private:
		...
		// VS constant register layout — Tomb Raider Legend
		// WVP at c0-c3, View batch at c8-c11, Proj batch at c12-c15
		// World is computed: WVP * inv(V*P), not uploaded directly.
		// We set world_start=0 so dirty tracking fires on WVP writes.
		int vs_reg_view_start_ = 8;
		int vs_reg_view_end_ = 12;
		int vs_reg_proj_start_ = 12;
		int vs_reg_proj_end_ = 16;
		int vs_reg_world_start_ = 0;
		int vs_reg_world_end_ = 4;
		int vs_reg_bone_threshold_ = 48;
		...
		// TRL passthrough: VP inverse cache for World = WVP * inv(VP)
		float cached_vp_inverse_[16] = {};
		float last_vp_[16] = {};
		bool vp_inverse_valid_ = false;

		// TRL game memory addresses for authoritative View and Projection
		static constexpr uintptr_t TRL_VIEW_MATRIX_ADDR = 0x010FC780;
		static constexpr uintptr_t TRL_PROJ_MATRIX_ADDR = 0x01002530;
