...
4. Added `try/except urllib.error.HTTPError` around all Anthropic API calls (`urllib.request.urlopen`) in `.github/workflows` to gracefully catch and print Anthropic 400 Bad Request / 401 Unauthorized errors instead of silently failing with Python stack traces.
...
The PR risk, review, and idea generation workflows were failing with `urllib.error.HTTPError: HTTP Error 400: Bad Request` likely due to invalid model strings or expired credentials. Capturing the HTTPError and updating model strings solves this and provides better logging.
