...
def handle_utility_commands(args: argparse.Namespace) -> None:
    """Handle utility commands that do not require running the agent loop."""
    if args.health:
...
def check_preflight(dry_run: bool) -> None:
    """Run preflight health checks before starting the agent loop."""
    if not dry_run:
        from gamepilot.health import run_all_checks
...
def print_result(result: dict) -> None:
    """Print the final result of the agent loop."""
    print(f"\n{'=' * 60}")
...
def main():
    ...
    # Health check and Runtime management commands
    handle_utility_commands(args)
