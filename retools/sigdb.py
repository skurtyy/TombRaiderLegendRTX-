...
def _categorize_name(name: str) -> str:
    ...
    for hint in _MATH_NAME_HINTS:
        if hint in name or hint.lower() in lower:
            return "math"
    return "unknown"
...
def extract_structural_sig(b, va: int):
    try:
...
class SignatureDB:
    ...
    def add_byte_sig(self, *, name, pattern, mask, func_size, tail_crc,
                     compiler="", source="", category=""):
    ...
    def add_structural_sig(self, *, name, block_count, edge_count, call_count,
    ...
    def match_bytes(self, code, func_size, preferred_compiler, func_tail_crc):
        if len(code) < 32:
            return []
        code32 = code[:32]
        ...
        for name, pattern, mask, db_size, db_tail_crc, compiler, source, category in cur:
            ...
            if preferred_compiler and compiler == preferred_compiler:
                score += 0.05
            candidates.append((
                Match(name=name, confidence=min(score, 1.0), tier="byte",
    ...
    def match_structural(self, block_count, edge_count, call_count,
                         mnemonic_hash, constants="", preferred_compiler=""):
        cur = self._conn.execute(
        ...
        for name, bc, ec, cc, mh2, compiler, source, category in cur:
            ...
            if preferred_compiler and compiler == preferred_compiler:
                score += 0.05
            candidates.append((
                Match(name=name, confidence=min(score, 1.0), tier="structural",
                      compiler=compiler, source=source, category=category),
                score,
            ))
        candidates.sort(key=lambda c: c[1], reverse=True)
    ...
    def identify(self, b, va, preferred_compiler=""):
    ...
    def scan(self, b, preferred_compiler=""):
        fp = self.fingerprint(b)
        compiler = preferred_compiler or fp.get("compiler", "")
        results = {}
...
def build_from_manifest(db, manifest: dict) -> int:
    added = 0
...
def main(argv=None):
    ...
    bp.add_argument("-o", "--output", default=str(DEFAULT_DB_PATH), help="Output DB path")
    bp.add_argument("--csv", nargs="*", default=[], help="Optional CSV address override files")

    sp = sub.add_parser("scan", help="Scan a binary for known functions")
    sp.add_argument("binary", help="PE binary path")
    sp.add_argument("-d", "--db", default=str(DEFAULT_DB_PATH), help="Signature DB path")
    ...
    ip.add_argument("-d", "--db", default=str(DEFAULT_DB_PATH), help="Signature DB path")
    ...
    pp.add_argument("--repo", default=_HF_REPO_DEFAULT, help="HuggingFace repo (owner/name)")
    pp.add_argument("--sources", action="store_true", help="Also pull source JSON files")
